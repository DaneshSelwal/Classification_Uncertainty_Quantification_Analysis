...
MODEL_NAME_MAP = {
    'DOFA_Spectral':     'DOFA_Spectral',
    'Hiera_ViT':         'Hiera_ViT',
    'DOFA_Hiera_Fusion': 'DOFA_Hiera_Fusion',
}

# Unsafe deserialization is allowed only for trusted model artifacts.
TRUSTED_MODEL_ROOTS = [
    Path('/content/drive/My Drive/Classification/dofa/models'),
...
def assert_environment_and_files():
    ...
    if missing:
        ...
        msg += '\n\nExpected names:\n- DOFA_Spectral_best.keras\n- Hiera_ViT_best.keras\n- DOFA_Hiera_Fusion_best.keras'
...
class DOFAWavelengthEmbed(layers.Layer):
    """DOFA-inspired dynamic patch embedding: a small hypernetwork generates
    per-band projection weights conditioned on each band's central wavelength
    (Xiong et al., 2024 — DOFA). Must match the definition in Model_training.ipynb
    exactly, since it is deserialised from the saved .keras model files.
    """
    ...
    def build(self, input_shape):
        self.wavelengths = tf.constant(self.wavelengths_um, dtype=tf.float32)
    ...
    def call(self, patches):
        w      = tf.reshape(self.wavelengths, [-1, 1])
        kernel = self.hyper_out(self.hyper_fc2(self.hyper_fc1(w)))
...
Securely loads all three models (DOFA_Spectral, Hiera_ViT, DOFA_Hiera_Fusion) from trusted paths and runs a small forward pass to verify output shape and numerical sanity.
...
required_prefixes = {
    ...
    'Split_DOFA_Spectral',     'Split_Hiera_ViT',     'Split_DOFA_Hiera_Fusion',
    'ClassCond_DOFA_Spectral', 'ClassCond_Hiera_ViT', 'ClassCond_DOFA_Hiera_Fusion',
    'RC3P_DOFA_Spectral',      'RC3P_Hiera_ViT',      'RC3P_DOFA_Hiera_Fusion',
    'Clustered_DOFA_Spectral', 'Clustered_Hiera_ViT', 'Clustered_DOFA_Hiera_Fusion',
    'RAPS_DOFA_Spectral',      'RAPS_Hiera_ViT',      'RAPS_DOFA_Hiera_Fusion',
