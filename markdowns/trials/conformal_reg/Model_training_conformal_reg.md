...
TRAIN_CFG = {
    "label_smoothing": 0.05,   # retained for reference only; unused with the conformal regularizer
    "weight_decay":    1e-4,
    "clipnorm":        1.0,
    "cosine_alpha":    0.05,
}

# ── Conformal-training regularizer hyper-parameter ─────────────────────────
# lambda_reg: weight on the differentiable set-size penalty added on top of plain
#             cross-entropy. Higher values push false-class probability mass down
#             harder, which should make post-hoc Split/RAPS conformal sets tighter.
#             Typical range in the literature is roughly 0.05 - 0.2; we start at 0.1.
LAMBDA_REG = 0.1
...
## 5.1 — Conformal-training regularizer
A differentiable surrogate for conformal prediction set size (Stutz et al., 2022 style), added on top of plain cross-entropy. Rather than treating conformal calibration purely as a post-hoc step, this penalises the model during training whenever it puts meaningful probability mass on non-target classes -- which is exactly the quantity that inflates Split/RAPS conformal prediction sets afterwards. This is an isolated, additive change: it does not touch focal-loss-style example re-weighting, only the amount of probability mass left on the wrong classes.

```python
def categorical_conformal_loss(lambda_reg=0.1):
    """Cross-entropy + false-class-mass regularizer, for one-hot targets (GFNet, ViT).

    total_loss = CE(y_true, y_pred) + lambda_reg * sum_{c != true class} p_c
    """
...
def sparse_categorical_conformal_loss(lambda_reg=0.1):
    """Cross-entropy + false-class-mass regularizer, for integer targets (AlexNet legacy path)."""
    def loss_fn(y_true, y_pred):
        eps           = tf.keras.backend.epsilon()
        y_pred        = tf.clip_by_value(y_pred, eps, 1.0 - eps)
        ...
        false_class_mass = tf.reduce_sum(y_pred * (1.0 - y_true_onehot), axis=-1)
        return ce + lambda_reg * false_class_mass
    return loss_fn
```
...
def train_save_evaluate(model_name, model_builder, capacity_tag="max"):
    ...
    if model_name == "AlexNet_CNN":
        model.compile(
            ...
            loss=sparse_categorical_conformal_loss(lambda_reg=LAMBDA_REG),
            metrics=["accuracy"],
        )
        callbacks = [
        ...
        model.compile(
            ...
            loss=categorical_conformal_loss(lambda_reg=LAMBDA_REG),
            metrics=["accuracy"],
        )
        callbacks = [
            keras.callbacks.ModelCheckpoint(
