...
# alpha: balancing weight -- now CLASS-BALANCED (see Sec. 3.3 below) instead of a flat
#        scalar. FOCAL_ALPHA_FLAT is retained for reference only; unused now that
#        the per-class CB_FOCAL_ALPHA vector is active.
FOCAL_GAMMA      = 2.0
FOCAL_ALPHA_FLAT = 0.25   # retained for reference only; unused
CB_BETA          = 0.999  # effective-number-of-samples hyper-parameter (Cui et al., 2019)
...
## 3.3 — Class-Balanced Focal Loss alpha
Replaces the flat `FOCAL_ALPHA` scalar with a per-class weight vector derived from the
*effective number of samples* (Cui, Jia, Lin, Song & Belongie, 2019: "Class-Balanced Loss
Based on Effective Number of Samples"). Rarer classes receive a proportionally larger
weight in the focal loss, on top of focal loss's existing easy/hard-pixel down-weighting.
Computed once from the primary training split (`y_train`) and reused for all three models,
since all splits are stratified and therefore share the same underlying class proportions.

```python
# Effective number of samples: E_c = (1 - beta^n_c) / (1 - beta)
# Class weight:                 w_c = 1 / E_c   (renormalised so mean weight == 1)
...
Categorical and sparse-categorical focal loss (Lin et al., 2017). Down-weights the contribution of easy, high-confidence pixels during training so the optimiser spends relatively more effort on hard / ambiguous pixels, in place of plain (possibly label-smoothed) cross-entropy. `alpha` is now supplied as the per-class, class-balanced vector `CB_FOCAL_ALPHA` computed in Sec. 3.3, instead of a single flat scalar.
...
def train_save_evaluate(model_name, model_builder, capacity_tag="max"):
    ...
    if model_name == "AlexNet_CNN":
        model.compile(
            ...
            loss=sparse_categorical_focal_loss(gamma=FOCAL_GAMMA, alpha=CB_FOCAL_ALPHA),
            metrics=["accuracy"],
        )
        callbacks = [
        ...
        model.compile(
            ...
            loss=categorical_focal_loss(gamma=FOCAL_GAMMA, alpha=CB_FOCAL_ALPHA),
            metrics=["accuracy"],
        )
        callbacks = [
            keras.callbacks.ModelCheckpoint(
