...
# ── Paths ─────────────────────────────────────────────────────────────
MODEL_DIR   = Path("/content/drive/My Drive/Classification/edl/models")
RESULTS_DIR = Path("/content/drive/My Drive/Classification/edl/results")
...
def make_edl_loss(epoch_variable, total_epochs, num_classes, sparse=False):
    ...
    def loss_fn(y_true, y_pred):
        ...
        # KL-divergence term: penalises evidence accumulated on incorrect classes.
        # alpha_tilde 'removes' the true class's evidence (sets it to 1) so only
        ...
        kl_div = annealing_coef * tf.reduce_sum(
            tf.math.lgamma(alpha_tilde) - tf.math.lgamma(tf.ones_like(alpha_tilde)), axis=-1
        )
...
def train_save_evaluate(model_name, model_builder, capacity_tag="max"):
    ...
    if model_name == "AlexNet_CNN":
        model.compile(
            ...
            loss=make_edl_loss(edl_epoch_var, EPOCHS, num_classes, sparse=True),
            metrics=["accuracy"],
        )
        callbacks = [
        ...
        model.compile(
            ...
            loss=make_edl_loss(edl_epoch_var, EPOCHS, num_classes, sparse=False),
            metrics=["accuracy"],
        )
        callbacks = [
            keras.callbacks.ModelCheckpoint(
