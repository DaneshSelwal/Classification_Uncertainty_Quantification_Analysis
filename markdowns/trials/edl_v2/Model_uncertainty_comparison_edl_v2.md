...
TRUSTED_MODEL_ROOTS = [
    Path('/content/drive/My Drive/Classification/edl_v2/models'),
...
def evidence_activation(logits):
    ...
    Must be registered here too since it's baked into the saved model's architecture,
    not just its (unused, compile=False) loss."""
...
Securely loads all three models (AlexNet, GFNet, ViT) from trusted paths and runs a small forward pass to verify output shape and numerical sanity.
