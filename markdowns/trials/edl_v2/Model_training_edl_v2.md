...
# ── Paths ─────────────────────────────────────────────────────────────
MODEL_DIR   = Path("/content/drive/My Drive/Classification/edl_v2/models")
RESULTS_DIR = Path("/content/drive/My Drive/Classification/edl_v2/results")
...
TRAIN_CFG = {
    "label_smoothing": 0.05,   # retained for reference only; unused with the EDL loss
    "weight_decay":    1e-4,
    "clipnorm":        1.0,
    "cosine_alpha":    0.05,
}

# ── EDL fix: extra damping on the KL term, on top of epoch-based annealing ────
# The first EDL run collapsed (every model converged to a uniform 1/7 output on every
# pixel) because the KL term could grow far larger than the cross-entropy term. We now
# use the PROPER, bounded Dirichlet KL divergence (see 5.1 below) and additionally scale
# it down by KL_LAMBDA as a safety margin. Increase toward 1.0 only if evidence still
# looks too weak once this run completes.
KL_LAMBDA = 0.1
...
### Evidential output activation
For Evidential Deep Learning the final layer must output non-negative *evidence* rather than a softmax probability. We keep the architecture otherwise identical (same single final `Dense` layer, same layer count, so penultimate-layer feature extraction used elsewhere is unaffected) and only swap the activation function: `alpha = softplus(logits) + 1`. `alpha` is the Dirichlet concentration parameter per class; it is always ≥ 1 and is *not* normalised to sum to 1 (unlike softmax).

```python
@tf.keras.utils.register_keras_serializable()
def evidence_activation(logits):
    """Map raw logits to Dirichlet alpha parameters: alpha = softplus(logits) + 1.

    softplus(.) guarantees non-negative 'evidence'; adding 1 gives the alpha used to
    parameterise a Dirichlet distribution over the class-probability simplex (Sensoy et al., 2018).
    """
...
def build_alexnet(input_shape, num_classes, dropout_rate=0.25, cfg=None):
    ...
    outputs = layers.Dense(num_classes, activation=evidence_activation, name="alex_logits")(x)
...
def build_gfnet(input_shape, num_classes, inner_patch=3, hidden_dim=512,
    ...
    outputs = layers.Dense(num_classes, activation=evidence_activation, name="gf_logits")(x)
...
def build_vit_unet_singlehead(
    ...
    outputs = layers.Dense(num_classes, activation=evidence_activation, name="vit_logits")(y)
...
## 5.1 — Evidential Deep Learning (EDL) loss
Trains the model to output Dirichlet concentration parameters (`alpha`, produced by the `evidence_activation` final layer above) instead of a plain softmax distribution. The loss has two parts: an expected cross-entropy term that pushes `alpha` of the *correct* class up, and a KL-divergence regulariser that penalises the model for accumulating evidence on *incorrect* classes -- gradually phased in via an epoch-based annealing coefficient so early training is not destabilised (Sensoy et al., 2018). Because a standard `loss=...` argument to `model.compile()` has no way to see the current epoch number, `EDLAnnealer` is a small callback that writes the epoch into a shared `tf.Variable` that the loss closure reads from.

```python
class EDLAnnealer(keras.callbacks.Callback):
    """Writes the current epoch number into a shared tf.Variable each epoch, so the EDL loss's
    KL-annealing coefficient can be computed inside a standard compile()-style loss function."""
...
def make_edl_loss(epoch_variable, total_epochs, num_classes, sparse=False, kl_lambda=0.1):
    """Build an EDL loss closure. `y_pred` here is `alpha` (see evidence_activation above),
    NOT a softmax probability vector.

    Fix vs. the first EDL run: uses the PROPER Dirichlet KL divergence
    KL( Dir(alpha_tilde) || Dir(1,...,1) ) instead of the earlier unbounded
    sum(lgamma(alpha_tilde)) approximation, and applies an extra kl_lambda damping factor
    on top of epoch-based annealing. The earlier version let the KL term grow far larger
    than the cross-entropy term, which is why every model collapsed to a uniform 1/7
    output on every pixel (zero evidence for every class, everywhere).
    """
    total_epochs_f = tf.cast(total_epochs, tf.float32)
    ...
    def loss_fn(y_true, y_pred):
        ...
        # Expected cross-entropy: pushes alpha of the true class up
        loss_ce = tf.reduce_sum(y_true_oh * (tf.math.log(S) - tf.math.log(alpha)), axis=-1)

        # Proper (bounded) KL divergence: KL( Dir(alpha_tilde) || Dir(1,...,1) )
        # alpha_tilde 'removes' the true class's evidence (fixes it at 1) so only
        # non-target evidence is penalised.
        alpha_tilde = y_true_oh + (1.0 - y_true_oh) * alpha
        S_tilde     = tf.reduce_sum(alpha_tilde, axis=-1, keepdims=True)

        term1 = (
            tf.math.lgamma(tf.squeeze(S_tilde, axis=-1))
            - tf.math.lgamma(num_classes_f)
            - tf.reduce_sum(tf.math.lgamma(alpha_tilde), axis=-1)
        )
        term2 = tf.reduce_sum(
            (alpha_tilde - 1.0) * (tf.math.digamma(alpha_tilde) - tf.math.digamma(S_tilde)),
            axis=-1,
        )
        kl_proper = term1 + term2

        annealing_coef = tf.minimum(1.0, epoch_variable / (total_epochs_f / 2.0))
        kl_div = kl_lambda * annealing_coef * kl_proper

        return loss_ce + kl_div

    return loss_fn


def alpha_to_probs(alpha):
    """Convert Dirichlet alpha output to the expected class probabilities: p_hat = alpha / S."""
...
def train_save_evaluate(model_name, model_builder, capacity_tag="max"):
    ...
    # Shared epoch counter for the EDL loss's KL-annealing coefficient (see 5.1 above)
    edl_epoch_var = tf.Variable(0.0, trainable=False, dtype=tf.float32, name=f"{model_name}_edl_epoch")
    ...
    if model_name == "AlexNet_CNN":
        model.compile(
            optimizer=keras.optimizers.Adagrad(learning_rate=ALEXNET_LR_START, clipnorm=TRAIN_CFG["clipnorm"]),
            loss=make_edl_loss(edl_epoch_var, EPOCHS, num_classes, sparse=True, kl_lambda=KL_LAMBDA),
            metrics=["accuracy"],
        )
        callbacks = [
        ...
        model.compile(
            ...
            loss=make_edl_loss(edl_epoch_var, EPOCHS, num_classes, sparse=False, kl_lambda=KL_LAMBDA),
            metrics=["accuracy"],
        )
        callbacks = [
            keras.callbacks.ModelCheckpoint(
    ...
    # Model output is alpha (Dirichlet concentration), not a softmax probability --
    # convert to the expected probability p_hat = alpha / S before any downstream metric
    # (log-loss, Brier score, ECE, classification report) touches it.
    y_eval_prob = alpha_to_probs(model.predict(x_eval, batch_size=BATCH_SIZE, verbose=0))
    y_test_prob = alpha_to_probs(model.predict(x_te,   batch_size=BATCH_SIZE, verbose=0))
