...
# ── Paths ─────────────────────────────────────────────────────────────
MODEL_DIR   = Path("/content/drive/My Drive/Classification/cdl/models")
RESULTS_DIR = Path("/content/drive/My Drive/Classification/cdl/results")
...
TRAIN_CFG = {
    "label_smoothing": 0.05,   # retained for reference only; unused with the CDL loss
    "weight_decay":    1e-4,
    "clipnorm":        1.0,
    "cosine_alpha":    0.05,
}

# ── Conformalized Deep Learning (CDL) hyper-parameters ────────────────────
# CDL_MU:  weight of the uniform-matching (conformal) loss relative to cross-entropy.
#          Follows the "Training Uncertainty-Aware Classifiers with Conformalized Deep
#          Learning" paper's own default range (0.1-0.2).
# CDL_TAU: softness of the pairwise "is class c more likely than the true class" threshold
#          used to build a differentiable APS-style score (smaller = closer to a hard
#          threshold, but with weaker gradients).
CDL_MU  = 0.1
CDL_TAU = 0.05
...
## 5.1 — Conformalized Deep Learning (CDL) loss
Adapts *"Training Uncertainty-Aware Classifiers with Conformalized Deep Learning"* (Einbinder et al., 2022) to our TensorFlow/Keras, per-pixel-patch classification setup. Two simplifications versus the official PyTorch/`torchsort` implementation, both noted explicitly since they're deliberate engineering choices, not oversights:

1. **No differentiable-sorting library.** With only 7 classes, the APS cumulative-mass score for the *true* class can be built directly from a pairwise soft "is class c more likely than the true class" comparison (a sigmoid threshold) instead of a full soft-sort over all classes -- exact in spirit, far cheaper, and avoids needing `torchsort` / `fast_soft_sort` in TensorFlow.
2. **Single combined batch, not a train/calibration split within each batch.** The official code routes part of each batch through cross-entropy and a disjoint part through the uniform-matching loss. We instead compute both losses on the full batch and add them (a commonly used simplified variant) rather than adding custom batch-splitting logic to a `train_step` override.

Worth flagging up front: our per-pixel-patch setup is actually a *better* fit for this method than the professor's note suggests -- the note's memory-overhead warning is about flattening a full `(H, W)` image into one giant sort per training step; here, every training sample is already an independent pixel/patch, so each step's sort only ever has to handle one normal mini-batch (our `BATCH_SIZE`), not the whole scene.

```python
def aps_scores_soft(y_true_oh, probs, tau):
    """Differentiable APS-style nonconformity score for the true class of each sample.

    s_i = (sum over classes c more likely than the true class of p_c)  +  u_i * p_true

    The hard 'more likely than' indicator is replaced by a sigmoid soft-threshold so the
    score stays differentiable w.r.t. the model's output probabilities. u_i ~ Uniform(0,1)
    is the same randomised tie-break used in standard APS.
    """
    p_true  = tf.reduce_sum(probs * y_true_oh, axis=-1, keepdims=True)          # (batch, 1)
    soft_gt = tf.sigmoid((probs - p_true) / tau) * (1.0 - y_true_oh)           # zero out true class
    tail_mass = tf.reduce_sum(probs * soft_gt, axis=-1)                        # (batch,)
    u = tf.random.uniform(tf.shape(p_true), minval=0.0, maxval=1.0)
    return tail_mass + tf.squeeze(u, axis=-1) * tf.squeeze(p_true, axis=-1)    # (batch,)


def uniform_matching_loss(scores):
    """Penalise a batch's conformity scores for deviating from Uniform(0,1) order statistics --
    the training-time analogue of the post-hoc conformal-calibration guarantee. Uses a plain
    tf.sort: since each sorted output is a direct copy of one specific input, the gradient it
    passes back is exact (not an approximation), so no differentiable-sorting library is needed
    for this step either.
    """
    n = tf.shape(scores)[0]
    sorted_scores = tf.sort(scores)
    n_f    = tf.cast(n, tf.float32)
    ranks  = tf.range(1, n + 1, dtype=tf.float32)
    target = ranks / (n_f + 1.0)                     # expected uniform order statistics
    return tf.reduce_mean(tf.square(sorted_scores - target))


def categorical_cdl_loss(num_classes, mu=0.1, tau=0.05):
    """Cross-entropy + mu * uniform-matching loss, for one-hot targets (GFNet, ViT)."""
    def loss_fn(y_true, y_pred):
        eps   = tf.keras.backend.epsilon()
        probs = tf.clip_by_value(y_pred, eps, 1.0 - eps)
        ce = -tf.reduce_sum(y_true * tf.math.log(probs), axis=-1)   # per-sample
        scores   = aps_scores_soft(y_true, probs, tau=tau)
        matching = uniform_matching_loss(scores)                   # scalar, batch-level
        return ce + mu * matching
    return loss_fn


def sparse_categorical_cdl_loss(num_classes, mu=0.1, tau=0.05):
    """Cross-entropy + mu * uniform-matching loss, for integer targets (AlexNet legacy path)."""
    def loss_fn(y_true, y_pred):
        eps           = tf.keras.backend.epsilon()
        probs         = tf.clip_by_value(y_pred, eps, 1.0 - eps)
        ...
        scores   = aps_scores_soft(y_true_onehot, probs, tau=tau)
        matching = uniform_matching_loss(scores)
        return ce + mu * matching
    return loss_fn
```
...
def train_save_evaluate(model_name, model_builder, capacity_tag="max"):
    ...
    if model_name == "AlexNet_CNN":
        model.compile(
            ...
            loss=sparse_categorical_cdl_loss(num_classes, mu=CDL_MU, tau=CDL_TAU),
            metrics=["accuracy"],
        )
        callbacks = [
        ...
        model.compile(
            ...
            loss=categorical_cdl_loss(num_classes, mu=CDL_MU, tau=CDL_TAU),
            metrics=["accuracy"],
        )
        callbacks = [
            keras.callbacks.ModelCheckpoint(
