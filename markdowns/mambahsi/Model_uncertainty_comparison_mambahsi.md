...
MODEL_NAME_MAP = {
    'MambaHSI_Small': 'MambaSmall',
    'MambaHSI_Base':  'MambaBase',
    'MambaHSI_Large': 'MambaLarge',
}

# Unsafe deserialization is allowed only for trusted model artifacts.
TRUSTED_MODEL_ROOTS = [
    Path('/content/drive/My Drive/Classification/mambahsi/models'),
...
def assert_environment_and_files():
    ...
    if missing:
        ...
        msg += '\n\nExpected names:\n- MambaHSI_Small_best.keras\n- MambaHSI_Base_best.keras\n- MambaHSI_Large_best.keras'
...
class SpectralScanLayer(layers.Layer):
    ...
    Duplicated verbatim from Model_training_mambahsi.ipynb Section 4.2 so that
    keras.models.load_model can deserialise it here.
    """
    ...
    def call(self, x):
        decay       = tf.nn.sigmoid(self.log_decay)
        u           = self.in_proj(x)
        gate        = self.gate_proj(x)
        gated_input = u * gate

        scan_input = tf.transpose(gated_input, [1, 0, 2])

        def step(prev_state, cur_input):
            return decay * prev_state + cur_input

        init_state = tf.zeros_like(scan_input[0])
        states = tf.scan(step, scan_input, initializer=init_state)
...
class SelectiveSSMBlock(layers.Layer):
    """One MambaHSI-style block: bidirectional selective-scan + GELU MLP, both
    wrapped in pre-LN residual connections. Duplicated verbatim from
    Model_training_mambahsi.ipynb Section 4.2 so that keras.models.load_model
    can deserialise it here.
    """
...
Securely loads all three MambaHSI variants (Small, Base, Large) from trusted paths and runs a small forward pass to verify output shape and numerical sanity.
...
required_prefixes = {
    ...
    'Split_MambaSmall',     'Split_MambaBase',     'Split_MambaLarge',
    'ClassCond_MambaSmall', 'ClassCond_MambaBase', 'ClassCond_MambaLarge',
    'RC3P_MambaSmall',      'RC3P_MambaBase',      'RC3P_MambaLarge',
    'Clustered_MambaSmall', 'Clustered_MambaBase', 'Clustered_MambaLarge',
    'RAPS_MambaSmall',      'RAPS_MambaBase',      'RAPS_MambaLarge',
